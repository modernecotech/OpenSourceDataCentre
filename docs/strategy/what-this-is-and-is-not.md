# What This Is and Is Not

## What This Project Is

- A reference architecture for sovereign, sustainable datacentres.
- A practical build path from 50 kW edge sites to national AI-ready infrastructure.
- A set of open BOMs, calculators, software adapters, mechanical designs, and commissioning tests.
- A way for countries to reduce dependency on closed cloud platforms and proprietary datacentre management stacks.
- A structure for local training, maintenance, spares, and second-source procurement.

## What This Project Is Not

- It is not a certified datacentre design.
- It is not a substitute for licensed local engineers, fire engineers, electrical engineers, code approval, or authority review.
- It is not a promise that marketplace parts are safe, compliant, or suitable for life-safety and critical-power systems.
- It is not a hyperscaler clone.
- It is not a reason to skip commissioning, documentation, training, spare parts, security review, backup, or disaster recovery.

## Safety Boundary

The repository may describe topology, planning assumptions, open software integrations, mechanical artifacts, and commissioning procedures. It must not imply that unreviewed designs can be built without local engineering sign-off, code compliance, fire review, utility approval, occupational safety review, or insurance requirements.

## Certification Boundary

Use language such as "Tier II-like" or "Tier III-like" only to explain design intent around maintainability and fault tolerance. Do not call a site "Tier certified" unless a recognized certification body has audited and certified that site.
