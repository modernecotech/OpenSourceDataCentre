...
This repository is currently a strong architecture, planning framework, CSV-backed catalogue, calculator, and shallow Rust prototype. It is not yet a deployable national cloud platform.

The current portal is a small standard-library Rust HTTP server with in-process sample data and CSV-backed catalogues. It demonstrates the intended infrastructure workbench, tenant, operator, Edge Shield, config-editor, lifecycle, hardware provisioning, commercial, customer operations, assurance, developer, data-platform, and planner surfaces. Those consoles are wireframes and command-payload prototypes unless a page explicitly says otherwise.

Most adapters remain planning contracts. The first concrete read-first adapters are PowerDNS, NetBox, Keycloak, OpenBao, and Argo CD: `PowerDnsHttpAdapter` can call the PowerDNS Authoritative HTTP API and read zones and record sets, `NetBoxHttpAdapter` can call the NetBox REST API and read sites, racks, devices, and IP addresses, `KeycloakHttpAdapter` can call the Keycloak Admin REST API and read realms, groups, and realm roles, `OpenBaoHttpAdapter` can call the OpenBao HTTP API and read mounted secret engines and ACL policy names, and `ArgoCdHttpAdapter` can call the Argo CD API and read application sync and health status. They are documented in [PowerDNS Read Adapter](docs/software/powerdns-read-adapter.md), [NetBox Read Adapter](docs/software/netbox-read-adapter.md), [Keycloak Read Adapter](docs/software/keycloak-read-adapter.md), [OpenBao Read Adapter](docs/software/openbao-read-adapter.md), and [Argo CD Read Adapter](docs/software/argocd-read-adapter.md). They are not yet wired into the portal, do not write changes, and do not persist evidence. Forgejo PR creation, Flux, Proxmox, CloudStack, OpenStack, PostgreSQL runtime persistence, billing, and security integrations remain planned or prototype-only unless backed by explicit code and tests. The live-adapter roadmap is tracked in [live-adapter-roadmap.csv](data/software/live-adapter-roadmap.csv), proof commands are tracked in [live-adapter-proof-catalogue.csv](data/software/live-adapter-proof-catalogue.csv), both are exposed through the infrastructure workbench, and the path is documented in [Live Adapter Roadmap](docs/software/live-adapter-roadmap.md). The first PostgreSQL persistence migration for workflow state is [0001_osdc_portal_state.sql](crates/osdc-portal/migrations/0001_osdc_portal_state.sql), with its boundary documented in [Portal Persistence](docs/architecture/portal-persistence.md).
...
- Make PowerDNS, NetBox, Keycloak, OpenBao, and Argo CD the first credible verticals: read DNS, inventory, identity, secrets-control-plane, and GitOps sync state, write evidence objects, show that evidence in the workbench, and only then repeat the pattern for GitOps pull requests, cloud, and persistence adapters.
...
- [Live Adapter Roadmap](docs/software/live-adapter-roadmap.md), [PowerDNS Read Adapter](docs/software/powerdns-read-adapter.md), [NetBox Read Adapter](docs/software/netbox-read-adapter.md), [Keycloak Read Adapter](docs/software/keycloak-read-adapter.md), [OpenBao Read Adapter](docs/software/openbao-read-adapter.md), and [Argo CD Read Adapter](docs/software/argocd-read-adapter.md) - read-first path from plan-only adapters to real integrations, starting with narrow DNS, inventory, identity, secrets, and GitOps sync readers.
...
- [Adapter Crate](crates/osdc-adapters/) - real read-first PowerDNS, NetBox, Keycloak, OpenBao, and Argo CD HTTP adapters plus mostly plan-only trait skeletons for Forgejo/Flux writes, Proxmox, CloudStack, OpenStack, Kubernetes, Ceph, Redfish/OpenBMC, MAAS, Ironic, Metal3, Harbor, billing, MFA, and scanner/finding integrations.
